...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:guess_four) { described_class.new(0, 9, random_number, 4) }
      ...
      it 'is game over' do
        expect(guess_four).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:guess_four) { described_class.new(0, 9, random_number, 5) }
      
      it 'is not game over' do
        expect(guess_four).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guess_game_seven) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        guess_game_seven.update_range
        expect(guess_game_seven.min).to eq(guess_game_seven.max)
      end

      it 'does not update max' do
        guess_game_seven.update_range
        expect(guess_game_seven.max).to eq(8)
      end
    end
  end
end
